...
class UNet(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.class_embeded = class_embeded
        self.device = device
    ...
    def get_class_emb(self, x, c):
        new_x = torch.empty((0,4,32,32), device=self.device)  #, device='cuda'
        for i in range(len(c)):
            tmp = torch.concatenate((x[i], self.class_embeded[c[i]]), dim=0)
            new_x = torch.concatenate((new_x, tmp.view(1, 4, 32,32)))
        return new_x
        # new_x = x.clone()
        # for i in range(len(c)):
        #     new_x[i] = torch.concatenate((x[i], self.class_embeded[c[i]]), dim=0)
        # return new_x

    def forward(self, x, time, c):
        time_embed = self.time(time)
        x = self.get_class_emb(x, c)
